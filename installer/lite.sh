...
ln -s /usr/share/zoneinfo/Argentina/Misiones /etc/localtime &>/dev/null
...
post_reboot() {
  ...
  title -verd "ACTUALIZACION DEL SYSTEM OK"
  print_center -ama "Reboot y \nafter Continuamos!!!"
  msg -bar
}
...
install_start() {
  ...
  msg -bar
  print_center -ama "Paquetes de sistem .\n a instalar  confirmar.\n"
  msg -bar3
...
install_continue() {
  os_system
  msg -bar
  echo -e "      \e[5m\033[1;100m   COMPLETO PACKAGES Para SCRIPT   \033[1;37m"
  msg -bar
  ...
  dependencias
  msg -bar3
  print_center -azu "Removiendo obsolete packages"
